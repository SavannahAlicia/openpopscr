...
sim_and_fit <- function(true_par, n_occasions, n_sec_per_prim, mesh, seed = NA){
  ...
  AICdat <- data.frame(model = c("linear", "linear_wfirstprim", "smooth3df", "smooth3df_wfirstprim", "smooth6df", "smooth6df_wfirstprim"),
                       AIC = c(AIC(oo), AIC(oo2), AIC(oo3), AIC(oo4), AIC(oo5), AIC(oo6)))
  
  out <- list(plotdat, AICdat)
  names(out) <- c("plotdat", "AICdat")
  return(out)
}

test <- sim_and_fit(true_par, n_occasions, detectors, mesh, 19592)
plotdat <- test[[1]]
AICs <- test[[2]]
...
test2 <- sim_and_fit(true_par, n_occasions, detectors, mesh, 19593)
