...
#' @temporary_emigration if a 2x2 matrix with row sums to 1 (or list of such matrices, one per primary) then implements temporary emigration with the matrix giving transition probs, and a stationary starting distribution 
#'
#'
#' @return ScrData object 
#' @export
simulate_js_openscr <- function(par, detectors, mesh, primary,
                                ihp = NULL, move = FALSE, time = NULL, 
                                ne_trans = NULL, seed = NULL, print = TRUE,
                                temporary_emigration = NULL) {
  require(secr)
  n_occasions <- length(unique(primary))
  n_sec_occasions <- length(primary)
  ...
  if(is_te) {
    if(is.list(temporary_emigration)){
      if(length(temporary_emigration) != (n_occasions-1)){
        warning("temporary_emigration list must be of length n_occasions - 1")
        is_te <- FALSE
      } else {
        if(!all(unlist(lapply(temporary_emigration, is.matrix)))) {
          warning("temporary_emigration parameter must be a matrix or list of matrices")
          is_te <- FALSE
        } else {
          if(any(unlist(lapply(temporary_emigration, function(x){
            (nrow(x) != 2) | (ncol(x) != 2)})))) {
            warning("temporary_emigration matrices must be 2x2")
            is_te <- FALSE
          } else {
            if(any(unlist(lapply(temporary_emigration, function(x){
              sum(rowSums(x)) != 2})))) {
              warning("rows of the temporary emigration matrices must each sum to 1")
              is_te <- FALSE
            }
          }
        }
  ...
  if(is_te) {
    ...
    #SR: double checked with algebra
    if (!is.list(temporary_emigration)){
      temporary_emigration <- replicate(n_occasions, temporary_emigration,  simplify = F)
    }
    #note for stationary distribution we currently calculate the mean of tpms over primaries
    ...
    for(i in 1:nrow(pop)){
      for(j in 2:n_occasions){
        present[i, j] <- sample.int(2, 1, prob = temporary_emigration[[(j-1)]][present[i, (j-1)],])
      } 
    }
  }
  ...
  for (i in seq(n)) {
    ...
    for(j in 1:nrow(trapn)){
      #subset by alive and present, use primary to get secondary occasions
      
      
       capture_history[i, , j]  <- capture_history[i, , j] * life[i, ][primary] * present[i, ][primary]
    }
    #LT: Old code
